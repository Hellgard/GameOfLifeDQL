from source2.classEnum import Direction, Ressource
from source2.constants import Point, font, White, Red, Black, Blue, Green, Yellow, Purple, Cyan, Brown, Grey, BLOCK_SIZE, FPS, Forward
from source2.classInventory import Inventory
import pygame
from source2.classMap import Map
import time
from source2.classPlayer import Player
import random
import numpy as np

class GameOfLifeAI:
    def __init__(self, map_size=Point(60, 60)):
    ...
    def check_evolution(self):
        ...
        if addPlayer:
            ...
            new_player = Player(Point(player.get_position().x, player.get_position().y), self.map_size.x,
                                self.map_size.y, len(self.players) + 1, 1, 3360)
    ...
    def check_player_state(self, player):
        if player is not self.player:
            ...
            GameOver = False
            reward = 0
            if player.timeBeforeDeath <= 0 or player.evolve_time >= 800:
                reward -= 30
                print("player " + str(player.id) + " died")
                GameOver = True
            return GameOver, reward
