...
GameOfLifeAI* create_game_of_life_ai(int width, int height) {
    ...
    init_player(game->player, width, height, random, 1);
...
void check_evolution(GameOfLifeAI* game) {
    ...
    if (addPlayer) {
        ...
        init_player(new_player, game->map_size.x, game->map_size.y, rand() % 1212121, 1);
...
void check_next_move(GameOfLifeAI* game, Player* player, Ressource** resources, Point* player_pos) {
    game->time++;
...
void choose_direction(int* action, Player* player) {
