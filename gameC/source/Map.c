...
void generate_map(Map* map, Player ** players, int num_players) {
    ...
    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            ...
            if (!isPlayer) {
                generate_ressources(map, tile, &tile_size);
            }
...
void regenerate_ressources(Map* map, Player** players, int num_players) {
    ...
    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            ...
            if (!isPlayer) {
                generate_ressources(map, map->map[y] + x, NULL);
            }
        }
    }
}
...
void draw_map(Map* map, const char* filename) {
    ...
    for (int y = 0; y < map->height; y++) {
        for (int x = 0; x < map->width; x++) {
            ...
            Color color = get_predominant_color(tile, tile_size);
            for (int i = 0; i < tile_size; i++)
                if (tile[i] == Ressource_PLAYER) {
                    color = Black;
                }
            // Write color code to file
            fprintf(file, "\033[38;2;%d;%d;%dm█\033[0m", color.r, color.g, color.b);
        }
        fprintf(file, "\n");
    }
    fclose(file);
